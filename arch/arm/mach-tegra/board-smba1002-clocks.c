...
static __initdata struct tegra_clk_init_table smba1002_clk_init_table[] = {
    ...
    /* Master clock */
    { "clk_m", NULL, 32768, true}, /* must be always on - Frequency will be autodetected */
...
// { "pll_m_out1", "pll_m", 26000000, true}, /* always on - unused ?*/
    { "pll_m_out1", "pll_m", 266400000, true}, /* always on - unused ?*/
...
// { "pll_c_out1", "pll_c", 108000000, true}, /* must be always on - system clock */
    { "pll_c_out1", "pll_c", 120000000, true}, /* must be always on - system clock */
...
// { "pll_d_out0", "pll_d", 5000000, true}, /* hdmi clock */
    { "pll_d_out0", "pll_d", 500000, true}, /* hdmi clock */
    ...
    /* pll_x */
    { "pll_x", "clk_m", 456000000, true}, /* */
    { "cclk", "pll_x", 456000000, true},
    { "cpu", "cclk", 456000000, true},
...
// { "vi_sensor", "pll_c", 150000000, true}, /* tegra_camera */
    ...
    { "i2s1", "pll_a_out0", 11289600, true}, /* i2s.0 */
    { "i2s2", "pll_a_out0", 1881600, true}, /* i2s.1 */
    { "audio", "pll_a_out0", 6084507, true},
    { "audio_2x", "audio", 12169014, true},
    { "spdif_in", "pll_p", 6084507, true},
...
// { CDEV2, NULL, 26000000, false}, /* probably used as USB clock - perhaps 24mhz ?*/
    ...
    { "usbd", "clk_m", 666000000 , false}, /* fsl-tegra-udc , utmip-pad , tegra_ehci.0 , tegra_otg */
    { "usb2", "clk_m", 666000000 , false}, /* tegra_ehci.1 */
    { "usb3", "clk_m", 666000000 , false}, /* tegra_ehci.2 */
